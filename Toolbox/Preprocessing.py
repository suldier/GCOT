...
class Processor:
    ...
    def show_pre(self, gt, y_test_index, y_predicted, save_path=None):
        """
        view HSI classification results
        :param gt:
    ...
    def show_pre1(self, gt, y_test_index, y_predicted, save_path=None):
        """
        view HSI classification results
        :param gt:
        :param y_test_index: test index of excluding 0th classes
        ...
        mat_co = np.array([[1,1,1],[2,101,230], [254,102,24],[135,104,57],[1,254,2],[1,254,253],[254,1,2],[254,254,1],[134,81,214],[190,190,190],[102, 51, 102],[204, 51, 204],[102, 51, 51],[153,204,51], [102, 204, 153]])
 
        im_t = np.zeros((gt_map.shape[0], gt_map.shape[1] ,3),dtype=np.int32)
        for i in range( gt_map.shape[0]):
            for j in range( gt_map.shape[1] ):
                im_t[i, j, :] = mat_co[gt_map[i,j]]
 
        if save_path != None:
            plt.figure()
            plt.imshow(im_t)  # spectral
            plt.axis('off')
            plt.tight_layout()
            #plt.savefig(save_path, format='pdf', bbox_inches='tight')
            plt.savefig(save_path, format='pdf',  bbox_inches='tight')#, dpi=1000)
    ...
    def show_class_map(self, y_pre, y_indx, gt, show=True, save=False):
        ...
        ax.imshow(gt_pre_2d, cmap='nipy_spectral')  # spectral
        plt.axis('off')
        plt.tight_layout()
